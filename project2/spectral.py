...
def preprocess_and_cluster_spectral(filename):
    ...
    plot_with_tsne(X_scaled, best_labels, title=f'Spectral Clustering with t-SNE ({filename})')

    return best_n_clusters, best_silhouette_score, rand_index

print("Processing 'cho.txt' Dataset")
cho_n_clusters, cho_silhouette, cho_rand = preprocess_and_cluster_spectral('cho.txt')

print("\nProcessing 'iyer.txt' Dataset")
iyer_n_clusters, iyer_silhouette, iyer_rand = preprocess_and_cluster_spectral('iyer.txt')
